  ...
  async function timeOff(date: string, removeIt = false) {
    ...
    setTimeOffs((list) =>
      removeIt
        ...
        : [...list.filter((x) => x.date !== date), { date, reason: offReason }].sort((a, b) => a.date.localeCompare(b.date)),
    );
    setOffDate("");
    setOffReason("");
    router.refresh();
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        <div className="flex items-center justify-between border-b border-white/[.06] px-6 py-5">
          <h2 id={`${uid}-title`} className="font-display text-lg font-semibold">
            {master ? tm.editTitle : tm.newTitle}
          </h2>
          <button onClick={onClose} aria-label={t.common.close} className="rounded-full p-2 text-fog hover:bg-white/5">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 space-y-7 overflow-y-auto px-6 py-6">
            ...
            <div className="flex flex-wrap gap-2">
              {COLORS.map((c) => (
                <button
                  ...
                  className={clsx("h-7 w-7 rounded-full transition", d.color === c && "ring-2 ring-white ring-offset-2 ring-offset-coal")}
                  style={{ background: c }}
                  aria-label={c}
                />
              ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input id={`${uid}-name`} className="input" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} />
            </div>
            <div>
              <label htmlFor={`${uid}-latin`} className="label">
                {tm.nameLatin}
              </label>
              <input
                id={`${uid}-latin`}
                className="input"
                value={d.nameLatin}
                onChange={(e) => setD({ ...d, nameLatin: e.target.value })}
              />
            </div>
            <div className="sm:col-span-2">
              <TranslateBar
                ...
                onResult={(r) =>
                  setD((x) => ({ ...mergeTranslations(x, r, ["specialty", "bio"]), nameLatin: x.nameLatin || r.latin.name || "" }))
                }
              />
            </div>
            <div className="sm:col-span-2">
              <label htmlFor={`${uid}-specialty`} className="label">
                {tm.specialty} · {lang.toUpperCase()}
              </label>
              <input
                id={`${uid}-specialty`}
                className="input"
                lang={lang}
                value={d[textKey("specialty")]}
                placeholder={lang !== "ru" ? d.specialty : ""}
                onChange={(e) => setD({ ...d, [textKey("specialty")]: e.target.value })}
              />
            </div>
            <div className="sm:col-span-2">
              <label htmlFor={`${uid}-bio`} className="label">
                {tm.bio} · {lang.toUpperCase()}
              </label>
              <textarea
                id={`${uid}-bio`}
                ...
                lang={lang}
                value={d[textKey("bio")]}
                placeholder={lang !== "ru" ? d.bio : ""}
                onChange={(e) => setD({ ...d, [textKey("bio")]: e.target.value })}
              />
            </div>
                ...
                return (
                  <div
                    ...
                    className={clsx(
                      ...
                      s ? "border-white/10 bg-white/[.03]" : "border-transparent",
                    )}
                  >
                    <button
                    ...
                    >
                      <span
                        className={clsx("absolute top-0.5 h-5 w-5 rounded-full bg-white transition-all", s ? "left-5.5" : "left-0.5")}
                      />
                    </button>
                    ...
                    {s ? (
                      <div className="ml-auto flex items-center gap-2 text-sm">
                        <input
                          type="time"
                          step={1800}
                          value={minToHHMM(s.startMin)}
                          onChange={(e) => setDay(wd, { startMin: hhmmToMin(e.target.value) })}
                        ...
                        <input
                          type="time"
                          step={1800}
                          value={minToHHMM(s.endMin)}
                          onChange={(e) => setDay(wd, { endMin: hhmmToMin(e.target.value) })}
                          className="input !w-auto !px-3 !py-1.5"
                        />
                      </div>
                    ) : (
          ...
          {master && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                />
                <button onClick={() => timeOff(offDate)} disabled={!offDate} className="btn-ghost !py-2 text-sm">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {timeOffs.map((to) => (
                  <span key={to.date} className="flex items-center gap-2 rounded-full bg-red-400/10 py-1 pr-1.5 pl-3 text-sm text-red-200">
                    {formatDate(to.date, locale)}
                    {to.reason && ` · ${to.reason}`}
                    <button
                      onClick={() => timeOff(to.date, true)}
                      ...
                      .map((s) => {
                        ...
                        return (
                          <button
                            key={s.id}
                            onClick={() =>
                              setD({ ...d, serviceIds: on ? d.serviceIds.filter((x) => x !== s.id) : [...d.serviceIds, s.id] })
                            }
        ...
        <div className="flex items-center gap-3 border-t border-white/[.06] px-6 py-4">
          {master && (
            <button onClick={remove} className="rounded-full p-3 text-red-300 hover:bg-red-400/10" aria-label={t.common.delete}>
          ...
          <button onClick={save} disabled={saving || d.name.trim().length < 2 || d.specialty.trim().length < 2} className="btn-forge">
