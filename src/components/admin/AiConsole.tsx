    ...
    <div className="space-y-1.5">
      {text.split("\n").map((line, i) => {
        ...
        const content = parts.map((p, j) =>
          p.startsWith("**") ? (
            <strong key={j} className="text-bone">
              {p.slice(2, -2)}
            </strong>
          ) : (
            p
  ...
  const { msgs, input, setInput, loading, send, listRef } = useChat({
    ...
    toReply: (res, data: { text: string; provider: string; model: string; fallbacks?: number; error?: string }) =>
      res.ok
        ? {
            content: data.text,
            meta: `${data.provider} · ${data.model}${data.fallbacks ? ` · ${tpl(ta.switches, { n: data.fallbacks })}` : ""}`,
          }
        ...
        <div ref={listRef} className="flex-1 space-y-4 overflow-y-auto p-6">
          {!msgs.length && (
            <div className="grid h-full place-items-center text-center">
              <div>
                <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-forge to-ember text-black">
                ...
                <div className="mt-6 flex max-w-xl flex-wrap justify-center gap-2">
                  {ta.prompts.map((p) => (
                    <button
                      key={p}
                      onClick={() => send(p)}
                      className="rounded-full border border-white/10 px-3.5 py-2 text-sm text-fog transition hover:border-forge/60 hover:text-bone"
                    >
                      {p}
                    </button>
                  ))}
            ...
            <div key={i} className={clsx("flex", m.role === "user" ? "justify-end" : "justify-start")}>
              <div
                className={clsx(
                  ...
                  m.role === "user"
                    ? "rounded-br-md bg-forge text-black"
                    : "rounded-bl-md border border-white/8 bg-white/[.03] text-bone/90",
                )}
              >
          ...
          {loading && (
            <div className="flex items-center gap-2 text-sm text-fog">
              <Spinner className="text-forge" /> {ta.thinking}
            </div>
          )}
        </div>
        <form
          ...
          className="flex gap-2 border-t border-white/[.06] p-3"
        >
          <input value={input} onChange={(e) => setInput(e.target.value)} placeholder={ta.placeholder} className="input" />
          <button disabled={!input.trim() || loading} className="btn-forge h-12 w-12 shrink-0 !p-0" aria-label={t.common.send}>
      ...
      <div className="space-y-6">
        <div className="card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <IconButton onClick={loadStatus} label={t.common.refresh}>
            ...
            {status?.chain.map((c) => {
              ...
              return (
                <div key={c.key} className="flex items-center gap-3 rounded-xl border border-white/[.06] bg-white/[.02] px-3 py-2.5">
                  <span className="grid h-6 w-6 shrink-0 place-items-center rounded-md bg-white/[.06] text-xs font-semibold">
                    {c.priority}
                  </span>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-fog">
                      {c.provider}
                      {c.lastError && c.state !== "ready" ? ` · ${c.lastError.slice(0, 40)}` : ""}
                    </div>
                  </div>
                  <span className={clsx("flex shrink-0 items-center gap-1 text-xs", style.cls)}>
                    <style.icon className="h-3.5 w-3.5" /> {label}
                    {c.state === "cooldown" ? ` ${c.cooldownSec}s` : ""}
                  </span>
                </div>
              );
            })}
          ...
          <div className="max-h-80 space-y-1 overflow-y-auto text-xs">
            ...
            {status?.logs.map((l) => (
              <div key={l.id} className="flex items-start gap-2 rounded-lg px-2 py-1.5 hover:bg-white/[.03]" title={l.error}>
                {l.ok ? (
                  <CircleCheck className="mt-px h-3.5 w-3.5 shrink-0 text-emerald-300" />
                ) : (
                  <CircleAlert className="mt-px h-3.5 w-3.5 shrink-0 text-red-300" />
                )}
                <div className="min-w-0 flex-1">
                  <div className="truncate">
                    {l.model} <span className="text-fog">· {l.channel}</span>
                  </div>
                ...
                <span className="shrink-0 tabular-nums text-fog">
                  ...
                  {new Date(l.createdAt).toLocaleTimeString(locale === "en" ? "en-GB" : "ru-RU", { hour: "2-digit", minute: "2-digit" })}
                </span>
              </div>
            ))}
          </div>
