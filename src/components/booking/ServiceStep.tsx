        ...
        {[null, ...p.categories].map((c) => (
          <button
            ...
            className={clsx(
              ...
              p.category === c ? "border-forge bg-forge/15 text-bone" : "border-white/10 text-fog hover:text-bone",
            )}
          >
            {c ?? t.common.all}
          </button>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {p.services.map((s) => (
          <button
            ...
            onClick={() => p.onPick(s)}
            className={clsx(
              "card group p-5 text-left transition hover:-translate-y-0.5 hover:border-forge/50",
              p.serviceId === s.id && "!border-forge ring-4 ring-forge/10",
            )}
          >
            ...
            <div className="mt-4 flex items-end justify-between text-sm">
              <span className="flex items-center gap-1.5 pb-0.5 text-fog">
