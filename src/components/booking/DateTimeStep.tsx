    ...
    <div className="space-y-5">
      <div>
        ...
        <div className="scrollbar-none -mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
          <button
            onClick={() => p.onMasterFilter(null)}
            className={clsx(
              "flex shrink-0 items-center gap-2.5 rounded-2xl border py-2 pr-4 pl-2 text-sm transition",
              !p.masterFilter ? "border-forge bg-forge/10" : "border-white/10 hover:border-white/25",
            )}
          >
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/[.06]">
          ...
          {p.serviceMasters.map((m) => (
            <button
              key={m.id}
              onClick={() => p.onMasterFilter(m.id)}
              className={clsx(
                "flex shrink-0 items-center gap-2.5 rounded-2xl border py-2 pr-4 pl-2 text-left text-sm transition",
                p.masterFilter === m.id ? "border-forge bg-forge/10" : "border-white/10 hover:border-white/25",
              )}
            >
              ...
              <span>
                ...
                <span className="text-xs text-fog">{m.specialty}</span>
              </span>
            </button>
          ))}
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {p.today && (
          <Calendar
            today={p.today}
            maxDate={addDays(p.today, 60)}
            availability={p.availability}
            selected={p.date}
            onSelect={p.onDate}
            loading={p.availLoading}
          />
        )}
            ...
            <div>
              ...
              <div className="space-y-4">
                {groups.map((g) => {
                  ...
                  return (
                    <div key={g.label}>
                      <div className="mb-2 flex items-center gap-1.5 text-xs uppercase tracking-wider text-fog">
                      ...
                      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                        {list.map((s) => (
                          <button
                            ...
                            className={clsx(
                              "rounded-xl border py-2.5 text-sm font-semibold tabular-nums transition",
                              p.time === s.time
                                ? "border-transparent bg-gradient-to-br from-forge to-ember text-black shadow-[0_6px_20px_-6px_rgba(255,90,31,.8)]"
                                : "border-white/10 bg-white/[.02] hover:border-forge/60",
                            )}
                          >
                            {minToHHMM(s.time)}
                          </button>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {p.slotMasters.map((m) => (
              <button
                key={m.id}
                onClick={() => p.onMaster(m.id)}
                className={clsx(
                  "card flex items-center gap-3 p-3 text-left transition",
                  p.masterId === m.id ? "!border-forge ring-4 ring-forge/10" : "hover:border-white/20",
                )}
              >
                <Avatar name={m.name} color={m.color} size={44} />
                <div className="flex-1">
                  <div className="font-semibold">{m.name}</div>
                  <div className="text-xs text-fog">{m.specialty}</div>
                </div>
