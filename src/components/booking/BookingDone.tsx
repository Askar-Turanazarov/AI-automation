...
export function BookingDone({ done, inTelegram, onClose, onAgain }: { done: Done; inTelegram: boolean; onClose: () => void; onAgain: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="card mx-auto max-w-xl p-8 text-center sm:p-12">
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        {inTelegram ? (
          <button onClick={onClose} className="btn-forge">{b.ok}</button>
        ) : (
          <Link href={`/${locale}`} className="btn-forge">{b.home}</Link>
        )}
        <button onClick={onAgain} className="btn-ghost">{b.again}</button>
      </div>
    </motion.div>
  );
}
