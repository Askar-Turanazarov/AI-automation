...
export default async function BookingsPage({
  ...
  searchParams: Promise<{ date?: string; master?: string; range?: string }>;
}) {
  ...
  const q = (patch: Record<string, string | undefined>) => {
    const p = new URLSearchParams(
      Object.entries({ range, master: sp.master, date: sp.date, ...patch }).filter(([, v]) => v) as [string, string][],
    );
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        {[
          ...
          <Link key={k} href={q({ range: k, date: undefined })} className={chipClass(range === k && !sp.date, "px-4 py-2")}>
            {l}
          </Link>
        ))}
        ...
        <Link href={q({ master: undefined })} className={chipClass(!sp.master, "px-3 py-2")}>
          {tb.allMasters}
        </Link>
        {masters.map((m) => {
          ...
          return (
            <Link
              key={m.id}
              href={q({ master: m.id })}
              className={chipClass(sp.master === m.id, "flex items-center gap-2 py-1.5 pr-3 pl-1.5")}
            >
          ...
          return (
            <section key={date}>
              <div className="mb-3 flex flex-wrap items-baseline gap-x-3">
                <h2 className={clsx("font-display font-semibold", date === today && "text-forge")}>
                  {date === today ? tb.todayPrefix : ""}
                ...
                <span className="text-sm text-fog">
                  {tpl(tb.dayStats, {
                    n: active.length,
                    sum: formatUZS(
                      active.reduce((a, b) => a + b.service.price, 0),
                      locale,
                    ),
                  })}
                </span>
                  ...
                  return (
                    ...
                    >
                      <div className="font-display font-semibold tabular-nums">
                        ...
                        <span className="text-fog">–{minToHHMM(b.endMin)}</span>
                      </div>
                      <div>
                      ...
                      <div className="flex items-center gap-2">
                        <Avatar name={master} color={b.master.color} size={28} className="!rounded-lg" /> {master}
                      </div>
                      <div>
                        <div>
                          {b.clientName} ·{" "}
                          <a href={`tel:${b.phone.replace(/\s/g, "")}`} className="text-fog hover:text-bone">
                            {b.phone}
                          </a>
                        </div>
